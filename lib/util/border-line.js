 ...
 * 個別ポイントと合計ポイントを計算。
 */
BorderLine.prototype.calc = function (curPart, skillComb) {
    var i, len, restPart;
    ...
    for (var tree in this.eachSkill) {
        ...
        for (i = start, len = parts.length; i < len; ++i) {
            restPart = parts[i];
            point -= maxPtSet[restPart];
        }

        ret[curPart][tree] = point;
    }

    // 合計のボーダーラインを設定
    ret.sum = this.goalSum - Skill.sum(skillComb);
    for (i = start, len = parts.length; i < len; ++i) {
        restPart = parts[i];
        ret.sum -= this.maxSumSet[restPart];
    }
