...
Comb.activates = function (skillComb, goal) {
    if (goal == null) throw 'goal is required';
...
Comb.calcBorderLine = function (combsSet, skillNames, subtractedSet) {
    ...
    if (subtractedSet) {
        var subSC = this.unify(subtractedSet);
...
Comb._unifyList = function (list) {
    ...
    for (var i = 0, len = list.length; i < len; ++i) {
        ...
        if (skillComb['胴系統倍化']) skillComb = bodySC;
        newList.push(skillComb);
    }

    return Skill.merge(newList);
};
