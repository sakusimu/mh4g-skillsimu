 ...
 * 合計ポイントを計算。
 */
exports.calcBorderLine = function (combsSet, skillNames, subtractedSet) {
    ...
    if (subtractedSet) {
        var subSC = sutil.unify(subtractedSet);
        for (tree in goal) goal[tree] -= subSC[tree] || 0;
    }

    var goalSum = sutil.sum(goal);
    ...
    for (i = 0, len = parts.length; i < len; ++i) {
        ...
        restPt = sutil.sum(maxSumSet);
        ret.sum[part] = goalSum - restPt;
    }
 ...
 */
exports._calcMaxEachSkillPoint = function (combsSet) {
 ...
 * 部位ごとに、最大の合計ポイントを計算。
 */
exports._calcMaxSumSkillPoint = function (combsSet) {
    if (combsSet == null) return null;
 ...
 *     => { '攻撃': 20, '斬れ味': 10 }
 */
exports.goal = function (skillNames) {
...
exports.isOverMaxEachSkill = function (skillComb, borderLine, part) {
...
exports.isOverMaxSumSkill = function (skillComb, borderLine, part) {
    var sum = sutil.sum(skillComb);
...
/**
 * combine をする際に付いた不要なプロパティを取り除く。
 */
exports.brushUp = function (sets) {
    var ret = [];
    for (var i = 0, len = sets.length; i < len; ++i) {
        var decombSet = sets[i],
            set       = {};
