...
Combinator.prototype.combine = function (skillNames, bulksSet, equip) {
    ...
    var borderLine = util.comb.calcBorderLine(bulksSet, skillNames, equip),
        decombSets = [];

    // 装飾品無しでスキルが発動してるか
    if (util.comb.activates({}, borderLine.goal)) {
        decombSets.push({});
    } else {
        decombSets = this._combine(bulksSet, borderLine);
    }

    decombSets = util.comb.brushUp(decombSets);
...
Combinator.prototype._combineDecomb = function (decombSet, decombs, borderLine, part) {
    ...
    for (var i = 0, len = decombs.length; i < len; ++i) {
        ...
        var sc = util.skill.merge(decombSet.cache, skillComb);

        var set = util.clone(decombSet);
        set.cache = sc;
        set[part] = decomb;

        if (util.comb.activates(sc, borderLine.goal)) {
            ret.push(set);
            set.activates = true;
            slot = decomb.slot;
        ...
        var isOverMaxSum  = util.comb.isOverMaxSumSkill(sc, borderLine, part),
            isOverMaxEach = util.comb.isOverMaxEachSkill(sc, borderLine, part);
...
Combinator.prototype._removeOverlap = function (decombSets) {
    ...
    for (var i = 0, len = decombSets.length; i < len; ++i) {
        ...
        for (var part in decombSet) {
            ...
            names = names.concat(decomb.decos);
        }
...
Combinator.prototype._getJustActivated = function (decombSets, goal) {
    ...
    for (var i = 0, len = decombSets.length; i < len; ++i) {
        var decombSet = decombSets[i],
            skillComb = util.skill.unify(decombSet);
        if (util.comb.justActivates(skillComb, goal)) ret.push(decombSet);
    }
    return ret;
};
