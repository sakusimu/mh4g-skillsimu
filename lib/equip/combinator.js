(function (define) {
'use strict';
var deps = [ '../equip.js', '../util/skill.js', '../util/comb.js' ];
define(deps, function (Equip, Skill, Util) {
...
Combinator.prototype.combine = function (skillNames, norCombsSet) {
    ...
    var borderLine = Util.calcBorderLine(norCombsSet, skillNames);
    ...
    for (i = 0, ilen = norCombsSet.body.length; i < ilen; ++i) {
        ...
        for (j = 0, jlen = restParts.length; j < jlen; ++j) {
            part = restParts[j];
            var norCombs = norCombsSet[part] || null,
                seen     = [];
...
Combinator.prototype._combine = function (set, norCombs, borderLine, part) {
    ...
    if (norCombs == null || norCombs.length === 0) {
        ret.push(set.concat(null));
        return ret;
    }

    var unifiedSC = Skill.unify(set),
        bodySC    = part !== 'body' ? set[0].skillComb : null; // 胴系統倍化

    for (var i = 0, len = norCombs.length; i < len; ++i) {
        var norComb   = norCombs[i],
            skillComb = norComb.skillComb;

        if (skillComb['胴系統倍化'] > 0) skillComb = bodySC;

        var sc = Skill.merge(unifiedSC, skillComb);
        var isOverMaxSum = Util.isOverMaxSumSkill(sc, borderLine, part);
        ...
        var isOverMaxEach = Util.isOverMaxEachSkill(sc, borderLine, part);

        // 頑シミュさんの「打ち切る」は、処理対象のデータとして push しないことで実現
        if (isOverMaxSum && isOverMaxEach) ret.push(set.concat(norComb));
    }
...
return Equip.Combinator = Combinator;
   ...
   typeof module !== 'undefined' && module.exports ?
       ...
       function (deps, factory) {
           factory(this.simu.Equip, this.simu.Util.Skill, this.simu.Util.Comb);
       }
);
