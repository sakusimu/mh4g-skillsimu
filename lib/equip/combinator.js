...
Combinator.prototype.combine = function (skillNames, bulksSet) {
    ...
    combSets = this._combineUsedSlot0(skillNames, bulksSet);
    ret = Comb.brushUp(combSets);

    if (ret.length) return ret;

    combSets = this._combine(skillNames, bulksSet);
    ret = Comb.brushUp(combSets);
...
Combinator.prototype._combineUsedSlot0 = function (skillNames, bulksSet) {
    ...
    for (var i = 0, len = bulksSets.length; i < len; ++i) {
        var combSets = this._combine(skillNames, bulksSets[i]);
...
Combinator.prototype._combine = function (skillNames, bulksSet) {
    ...
    for (var i = 0, ilen = parts.length; i < ilen; ++i) {
        ...
        for (var j = 0, jlen = combSets.length; j < jlen; ++j) {
            ...
            var sets = this._combineEquip(combSet, bulks, borderLine, part);
...
Combinator.prototype._combineEquip = function (combSet, bulks, borderLine, part) {
    combSet = combSet || {};
    ...
    for (var i = 0, len = bulks.length; i < len; ++i) {
        ...
        // (ただし胴系統倍化は中断しない)
        if (!isOverSum && !(i === 0 && !!bulk.skillComb['胴系統倍化'])) break;
