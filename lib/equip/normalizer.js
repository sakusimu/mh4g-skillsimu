...
Normalizer.prototype._normalize1 = function (equips, skillTrees) {
    ...
    var fix = equips.length === 1; // 対象となる装備が1つの場合は固定装備と判断
    ...
    for (var i = 0, ilen = equips.length; i < ilen; ++i) {
        ...
        if (!fix && equip.skillComb['胴系統倍化']) {
            if (checkedTorsoUp) continue;
            ret['胴系統倍化'] = [ equip.skillComb ];
            checkedTorsoUp = true;
            continue;
        } else if (!fix && !Skill.contains(equip.skillComb, skillTrees)) {
...
Normalizer.prototype._normalize2 = function (combs, skillTrees) {
    if (combs == null || Object.keys(combs) === 0) return null;
    if (skillTrees == null || skillTrees.length === 0) return null;
