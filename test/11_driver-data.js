...
QUnit.test('equips', function () {
    ...
    QUnit.equal(got, 307, 'equips.head.length');
    ...
    QUnit.equal(got, 305, 'equips.body.length');
    ...
    QUnit.equal(got, 304, 'equips.arm.length');
    got = equips.arm['ブレイブアーム,0,0'];
    exp = ["ブレイブアーム,0,0","ブレイブアーム",0,0,1,0,1,1,1,1,2,0,0,0,1,"採取",3,"運気",2,"体力",-2,"乗り",2,null,null,"竜骨【小】",1,null,null,null,null,null,null];
    QUnit.deepEqual(got, exp, 'equips.arm');

    got = _.keys(equips.waist).length;
    QUnit.equal(got, 303, 'equips.waist.length');
...
QUnit.test('decos', function () {
    ...
    QUnit.equal(got, 156, 'decos.length');
    got = decos['耐絶珠【１】'];
...
QUnit.test('skills', function () {
    ...
    QUnit.equal(got, 256, 'skills.length');
    got = skills['攻撃力UP【小】'];
    exp = ["攻撃力UP【小】","攻撃",10,0];
